    ...
    def IniciateDataTransformation(self):
        try:
            if self.data_validation_artifact.status:
                ...
                os.makedirs(dir_name,exist_ok=True)
                save_object(filepath=self.data_transformation_config.transformed_train_X_file_path,content=X_transformed_train)
                save_object(filepath=self.data_transformation_config.transformed_train_Y_file_path,content=y_train)
                logging.info("Transformed train array is store successfully")
                save_object(filepath=self.data_transformation_config.transformed_test_X_file_path,content=X_transformed_test)
                save_object(filepath=self.data_transformation_config.transformed_test_Y_file_path,content=y_test)
